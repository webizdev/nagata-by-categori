...
class ProductController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->has('category')) {
            $activeCategory = Category::with(['children', 'parent.parent'])->where('slug', $request->category)->first();

            if ($activeCategory) {
                // Build breadcrumb path
                $temp = $activeCategory;
                while ($temp) {
                    $categoryPath->prepend($temp);
                    $temp = $temp->parent;
                }

                // Hanya tampilkan produk jika kategori tidak memiliki anak (level terakhir)
        ...
        } else if (!$search && !$request->filled('brand') && !$request->filled('model')) {
            // Jika tidak ada kategori yang dipilih (dan tidak sedang mencari/filter motor), sembunyikan produk
            $productQuery->whereRaw('1 = 0');
        }
        ...
        return view('products.index', compact(
            'categories', 'products', 'activeCategory', 'categoryPath',
